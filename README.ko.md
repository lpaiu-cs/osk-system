...
보호영역은 선의의 실수를 되돌리는 장치이며, **vault에 임의로 쓸 수 있는 상대에
...
- **폴더 이름 하나에 기억 하나.** 세션 키는 저장소 폴더의 이름이다(Git 공통
  디렉터리의 부모라서 워크트리는 본 저장소로 접히고, Git 밖에서는 작업 폴더
  이름). 폴더 이름이 같은 저장소들은 Scope 기억과 전사 착지를 공유하고,
  최상위 Git 서브모듈은 모두 `modules` 키를 공유한다. v4 전까지는 폴더 이름을
  서로 다르게 둔다.
...
- **노드 본문에는 비밀값 필터가 없다.** 의도된 설계다. 필터는 raw 전사,
  ...
  비밀값을 적지 않는다.
- **보호영역은 보안 경계가 아니다.** 선의의 실수를 되돌리는 장치다
  ([Problem](#problem) 참고).
...
구획의 수용 기록도 함께 남긴다. 승인은 수용의 기록이지 효력의 요건이 아니다
