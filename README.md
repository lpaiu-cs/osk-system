...
| CLI 미설정·미설치·미로그인·구독 인증 불확실·판본 불일치 등 | **검토 경고**와 함께 현재 세션의 **UserPromptSubmit 9·15턴 통합**으로 전환 |
...
**캐시 재사용은 별도 검증 대상이다.** 같은 CLI끼리의 fork는 실측됐지만,
Codex 앱에서 별도 실행기로 넘긴 실험에서는 캐시 미적중이 관측되어 그 경로의
자동 활성화는 보류 중이다. 구현·설치·캐시 적중·실제 노드 성장을 구별한다.
