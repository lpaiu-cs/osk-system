...
2. **정식 릴리스**(시행령 §10 6항)는 정본 저장소의 대화형 단말에서 선언하며,
   ...
   — 릴리스에 담긴 전 파일(자신 제외)의 경로와 내용 해시다. 선언의 전제는
   검증기 PASS·비밀값 스캔·깨끗한 작업 트리이며, 선언은 커밋과 버전 태그로
   고정된다.
3. **갱신**은 릴리스를 임시 디렉터리에 실체화하고 비준증빙과 전수 대조한
   ...
   다루고 정본에 닿지 않는다.
4. 갱신의 적용 범위는 릴리스 안의 발행 매니페스트(§1 1항)가 정한다 — MAP
   대상은 적용하고, KEEP(정본 저장소 전용)은 건너뛰고, SKEL은 없는 자리에만
   골격을 만든다. **별도의 갱신 매니페스트는 두지 않는다.** 직전 릴리스에서
   관리하던 파일이 이번 릴리스에서 빠졌으면 인스턴스에서도 제거한다 — 안
   하면 하류가 정본과 다른 프레임워크를 실행한다. 관리 집합의 정본은 갱신
   저널(7항)이다.
5. **인스턴스 소유 바닥**: `= ` 접두 Space 루트 아래(골격 제외)·`_ledger/`·
   ...
   보호 구획으로 파고들거나 루트를 벗어나는 골격은 거부한다.
