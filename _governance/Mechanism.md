...
4. **접두 규칙**: `= ` 접두는 Space 루트의 강조 표기다. 밑줄 접두 구획에는
   노드를 두지 않는다 — 예외가 없다. 통치 구획의 통치 문서·사료는 노드가
   아닌 규범 문서이고 frontmatter를 두지 않는다(헌법 3조 6항 · 시행령 §10
   1항). 노드 군집은 2항의 표에 선언된 경로(`= Scope/`·`= Domain/`·
   `= Person/`과 그 하위 군집, `transit/`)뿐이다. 그 밖의 루트 디렉토리는
   엔진·저장소 지원 구획이며 노드를 두지 않는다.
