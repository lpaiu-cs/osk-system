...
7. 갱신의 파일 적용은 하나의 **크래시-안전 트랜잭션**이다 — 건드릴 파일의
   ...
   미완료 트랜잭션은 **다음 실행이 pre-image로 복구**한다(manifest 존재가 복구
   신호다) — 어느 쪽도 혼합 상태(프레임워크 절반만 새 판)를 남기지 않는다.
   `begin`·`done`은 크래시 감지 경계다. 갱신 저널은 `_ledger/update.jsonl`에 §3 1항의 공통 규율로
