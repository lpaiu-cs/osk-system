...
7. 갱신의 파일 적용은 하나의 **트랜잭션**이다 — 파일 조작을 백업을 뜨며 먼저
   전부 끝낸 뒤에만 적용 상태를 저널에 남긴다. 도중 실패하면 백업으로 되돌려
   혼합 상태(프레임워크 절반만 새 판)를 남기지 않는다. `begin`·`done`은 크래시
   감지 경계이며, 미완료 `begin`은 다음 실행이 재수렴시킨다(내용 동일 파일의
   기준선 재기록). 갱신 저널은 `_ledger/update.jsonl`에 §3 1항의 공통 규율로
