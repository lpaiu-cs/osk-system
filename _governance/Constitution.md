...
4. Predicate Edge는 선언 노드와 상대 노드가 모두 유효한 서명 상태일 때에만 효력을
    가진다. 효력이 없는 동안에는 Link와 같이 취급된다. Observation 노드와
    Workbench는 Link 사용만 허용한다.
5. **술어의 자격** — 술어는 엔진이 그 종류에 따라 구별된 행동을 수행할
   때에만 둔다. 새 술어는 기존 술어로 표현할 수 없는 구별된 엔진 행동의
   필요가 실제 사용에서 반복적으로 확인되고, 그 행동과 함께 비준될 때에만
   추가된다.
6. Predicate Edge는 노드 서명과 독립된 유효성 트랙을 따른다. Edge의 컴파일이나
   변경 자체는 노드의 서명 대상 상태를 바꾸지 않는다. Predicate Edge는 성립의
   **권한 근거**를 가져야 한다 — 관계를 규약된 표기로 명시한 서명 상태, 사용자의
   명시적 판단 또는 비준된 결정론적 컴파일 규칙. 엔진은 권한 근거에서 Predicate
   Edge를 컴파일하며, 별도의 확인 절차를 요구하지 않는다. 에이전트의 의미 추론만으로
   얻은 관계는 Link 또는 후보에 머문다. 모든 활성 Predicate Edge는 그 권한 근거로
   추적 가능해야 한다.
