...
6. **참조** — 노드 A가 다른 노드 B를 Link 또는 Predicate Edge로
   ...
   반대 방향, 즉 참조 받는 노드에서 참조 하는 노드 방향으로 전파된다(제11조 4항).
...
4. **서명 전파** — 노드가 새로운 상태로 재서명되면, 그 노드를 참조하는 노드에
   재검토가 전파된다. 전파는 봉인을 자동으로 풀지 않으며, 중심성 산입 방식은 관계의
   종류에 따라 별도로 정한다. 중심성이 측정하는 개정 비용은 이 전파 범위의 크기다.
