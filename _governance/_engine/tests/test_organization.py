...
def node(name, body="Reusable evidence", edges=None):
    out = write.create_node(name, name, body, "gpt-6-astra", space="00_Scope/W1", edges=edges)
    assert out["ok"], out
    wire("W1", name)
    return out
def wire(hub, name):
...
class OrganizationTests(unittest.TestCase):
    ...
    def test_existing_domain_review_is_separate_and_duplicate_pages_have_unique_ids(self):
        self.case("""
            request = dict(title='Principles',summary='Shared principles',body='General conclusions',
                           drafter='fixture',space='00_Domain/Principles')
            ...
            result = write.create_node('Rule','Qualified rule','Identical paragraph. '*1000,
                                       'fixture',space='00_Domain/Principles')
            assert result['ok']; wire('Principles','Rule')
            current = organization.snapshot('00_Domain/Principles')
            ...
            assert organization.pending()[0]['scope'] == '00_Domain/Principles'
            finish_review('00_Domain/Principles','Synthetic rule has one explicitly bounded condition')
            assert organization.plan('00_Domain/Principles')['status'] == 'complete'
            rejected(lambda: organization.plan('00_Person/think'))
        """)
    ...
    def test_source_navigation_and_raw_round_are_not_knowledge_nodes(self):
        self.case("""
            rp=core.ROOT/"00_Scope/W1/_raw/evidence.md"; rp.parent.mkdir(parents=True,exist_ok=True)
            rp.write_text("## 1"+chr(10)+"User evidence",encoding="utf-8")
            a=node("A","See [[00_Scope/W1/_raw/evidence.md#1]]",{"derived-from":"[[00_Scope/W1/_raw/evidence.md]]"})
            ...
            write.update_node("A",add_edges={"derived-from":"[[00_Scope/W1/_raw/evidence.md#1]]"},remove_edges={"derived-from":"[[00_Scope/W1/_raw/evidence.md]]"})
    ...
    def test_real_hub_placement_and_identity_preservation(self):
        self.case("""
            ...
            write.create_node("Branch","Branch","Two related facts","gpt-6-astra",space="00_Scope/W1/Branch")
            wire("W1","Branch")
            moved=write.move_nodes(["A","B"],"00_Scope/W1/Branch"); assert moved["ok"]
    ...
    def test_missing_intermediate_hub_is_found_without_local_state(self):
        self.case("""
            write.create_node("Gap","Gap","Intermediate entrance","gpt-6-astra",space="00_Scope/W1/Gap")
            write.create_node("Child","Child","Nested entrance","gpt-6-astra",space="00_Scope/W1/Gap/Child")
            write.create_node("A","A","Retained nested knowledge","gpt-6-astra",space="00_Scope/W1/Gap/Child")
            wire("Child","A")
            gap=write._live_locate("Gap",graph.Index()); original=gap.read_bytes(); gap.unlink()
            assert not organization._state_path().exists()
            job=organization.plan("W1")
            assert any(i["path"]=="00_Scope/W1/Gap" and i["hub"] is None for i in job["issues"]),job
    ...
    def test_partial_move_restarts_from_remaining_identical_nodes(self):
        self.case("""
            ...
            a=node("A"); b=node("B")
            write.create_node("Branch","Branch","Related facts","gpt-6-astra",space="00_Scope/W1/Branch")
            job=organization.plan("W1")
            ...
            with patch("osk.write._apply_move",side_effect=injected):
                out=write.move_nodes(["A","B"],"00_Scope/W1/Branch")
            ...
            out=write.move_nodes(out["remaining"],"00_Scope/W1/Branch"); assert out["ok"],out
            ...
            write.create_node("Other","Other","Another meaningful branch","gpt-6-astra",space="00_Scope/W1/Other")
            assert write.move_nodes(["A"],"00_Scope/W1/Other")["ok"]
    ...
    def test_relocation_keeps_history_and_rechecks_current_navigation(self):
        self.case("""
            ...
            out=distillation.create_node({"key":"retained","sources":["Source"],"hub":"W1"},title="Conclusion",summary="Conclusion",body="Mechanism with scope limits",drafter="gpt-6-astra",space="00_Scope/W1")
            assert out["distillation"]["status"]=="complete",out
            original=distillation._load("retained")
            write.create_node("Branch","Branch","Mechanism knowledge","gpt-6-astra",space="00_Scope/W1/Branch")
            assert write.move_nodes(["Source","Conclusion"],"00_Scope/W1/Branch")["ok"]
    ...
    def test_single_move_adapter_preserves_partial_failure_response(self):
        self.case("""
            ...
            with patch("osk.write._apply_move",side_effect=OSError("injected I/O failure")):
                failed=write.move_node("A","00_Scope/W1/Branch")
            ...
            assert write.move_node("A","00_Scope/W1/Branch")["ok"]
            assert not organization.snapshot("W1")["pending_moves"]
        """)
    ...
    def test_adversarial_empty_body_raw_coordinates_and_lost_id_restart(self):
        self.case("""
            ...
            for anchor in ("999","not-a-round"):
                ref="[[00_Scope/W1/_raw/evidence.md#"+anchor+"]]"
    ...
    def test_id_hub_navigation_matches_title_navigation(self):
        self.case("""
            s=node("Source")
            out=distillation.create_node({"key":"by-id","sources":["Source"],"hub":"W1"},title="Conclusion",summary="Conclusion",body="A bounded rule",drafter="gpt-6-astra",space="00_Scope/W1")
