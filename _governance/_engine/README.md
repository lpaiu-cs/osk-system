...
| `osk/setup.py`·`scripts/setup.py` | Mechanism §1-2 8항 — 설치 도구: `.venv` 준비(부트스트랩), 릴리스 기준선, 하네스 MCP·훅 등록과 해제(확인한 계획만, 백업, 이 vault의 osk 항목만), 고를 때만 fork CLI·정기 실행·동기화 데몬 |
| `osk/services.py` | 운영체제 서비스 등록 — 정기 실행·동기화 데몬을 작업 스케줄러·launchd·systemd에(이 vault의 스크립트를 부르는 등록만, 원래 정의 백업) |
...
PYTHONPATH=_governance/_engine .venv/bin/python -m osk.cli protect "00_Person/Delegation" --reason "..."  # 사용자 전속
PYTHONPATH=_governance/_engine .venv/bin/python -m osk.cli approve "00_Person/Delegation" --reason "..."  # 사용자 전속
# 동기화 데몬: SYNC_ENABLED=1 nohup .venv/bin/python _governance/_engine/sync_daemon.py &
```

## 검색·선택 열람·성장 후보

검색은 정확한 제목 일치를 먼저, 앞뒤 공백·대소문자 차이만 있는 일치를 그다음으로
둔다. 그 밖의 질의는 기존 BM25 순서를 유지한다. 응답의 `title`과 실제 BM25
`score`는 바꾸지 않으므로 제목 일치가 있으면 점수 내림차순과 다를 수 있다.
...
좌표가 아니다. 큰 범위는 `next_view`로 이어 읽는다. `body_chars`로 전체 길이를
알 수 있어 목차 없는 본문과 목차 상한 밖도 열람할 수 있다. 부분 응답의
`view_hash`가 달라지면 목차·좌표부터 다시 확인한다. 이 표식은 전문 덮어쓰기용
해시가 아니며, 전문 교체 전에는 기본 `read_node(name)`으로 다시 읽어야 한다.

성장기는 등록된 과거 계획에 없던 Scope 노드 판본(id·파일 해시)을 우선한다.
기존 판본의 비교에도 한 자리를 남기고, `limit=1`이면 등록된 계획마다 우선순위와
순환 선택을 번갈아 적용한다. 직전 계획의 즉시 재시도는 다른 후보 뒤로 보낸다.
처음에는 모든 판본이 미관측 상태이며, 계획에 실렸다는 사실을 실제 열람·증류
완료로 보지 않는다. 완료 판단은 기존의 현재 상태 검증과 처분 영수증이 맡는다.
노드가 한 번 선택됐어도 새 노드와의 조합은 계속 검토 대상이다.

이 선택은 에이전트에게 보내는 자료의 우선순위를 정한다. 전체 색인 스캔과
