...
`strawberry` 같은 단어는 BPE 병합 결과 하나의 글자 시퀀스가 아니라 **여러 서브워드
토큰 조각**으로 나뉜다(예: `st` + `raw` + `berry` 식 — 정확한 분할은 토크나이저·버전에
따라 다르다). 모델이 입력으로 보는 단위는 이 토큰 조각의 ID 배열이지 개별 글자 `r`이
아니다.

그래서 "strawberry에 `r`이 몇 개냐" 같은 **글자 세기 과제**는 토큰 경계 뒤에 글자 정보가
가려져 직접적이지 않다. 불가능한 것은 아니며(모델·문맥에 따라 맞히기도 한다), 실패
원인도 BPE 하나가 아니라 토큰화·문자 단위 접근 부족·산술적 카운팅 한계·학습 분포가
섞인 결과다. 다만 토큰화는 이런 단순해 보이는 글자 과제가 왜 의외로 취약한지를
설명하는 핵심 요인 중 하나다.
