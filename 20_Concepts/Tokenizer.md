...
토크나이저를 거치고 나면 모델이 다루는 단위는 개별 문자가 아니라 **토큰 ID 시퀀스**다.
`strawberry`처럼 한 단어가 여러 토큰 조각으로 묶이면(자세히는 [[Byte Pair Encoding]]),
모델 입장에서 글자 `r`은 토큰 안에 흡수돼 직접 보이지 않는다.

이 때문에 "단어 안의 글자 수 세기" 같은 과제는 본질적으로 간접적이 된다. 토큰화가
글자 단위 카운팅을 직접적이지 않게 만들기 때문이며, 카운팅이 불가능해진다는 뜻은
아니다. 다만 단순해 보이는 글자 과제가 왜 취약할 수 있는지를 설명해 준다.
