...
def worker(a):
    ...
    target = a.target or next((a.root / "00_Scope").glob("osk-system/*.md")).stem
    counts = {}
...
def fixture(a):
    ...
    for i in range(a.nodes):
        ...
        p = a.root / "00_Scope" / group / (name + ".md")
    ...
    raw = a.root / "00_Scope/Bench00/_raw/.records/sample.txt"
    raw.parent.mkdir(parents=True)
    raw.write_text("# 1\n\n사용자: 합성 원료\n에이전트: 관측 결과\n", encoding="utf-8")
    ledger = a.root / "00_Scope/Workbench/_ledger/bench.jsonl"
    ...
    with a.output.open("w", encoding="utf-8") as out:
        ...
        try:
            ...
            if a.mutate:
                ...
                paths = {"node": a.root / "00_Scope/Bench00/node-00020.md",
                         "raw": a.root / "00_Scope/Bench00/_raw/.records/sample.txt",
                         "ledger": a.root / "00_Scope/Workbench/_ledger/bench.jsonl"}
